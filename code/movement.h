Servo yawServo; // names the servo responsible for YAW rotation, 360 spin around
                // the base
...
int lastYawServoVal =
    90; // initialize variables to store the last value of each servo
int lastPitchServoVal = 90;
int lastRollServoVal = 90;

int pitchMoveSpeed =
    8; // this variable is the angle added to the pitch servo to control how
       // quickly the PITCH servo moves - try values between 3 and 10
int yawMoveSpeed =
    90; // this variable is the speed controller for the continuous movement of
        // the YAW servo motor. It is added or subtracted from the yawStopSpeed,
        // so 0 would mean full speed rotation in one direction, and 180 means
        // full rotation in the other. Try values between 10 and 90;
int yawStopSpeed = 90; // value to stop the yaw motor - keep this at 90
int rollMoveSpeed =
    90; // this variable is the speed controller for the continuous movement of
        // the ROLL servo motor. It is added or subtracted from the
        // rollStopSpeed, so 0 would mean full speed rotation in one direction,
        // and 180 means full rotation in the other. Keep this at 90 for best
        // performance / highest torque from the roll motor when firing.
int rollStopSpeed = 90; // value to stop the roll motor - keep this at 90

int yawPrecision =
    150; // this variable represents the time in milliseconds that the YAW motor
...
int rollPrecision =
    158; // this variable represents the time in milliseconds that the ROLL
         ...
         // value down or up slightly, but it should remain around the stock
         // value (160ish) for best results.

int pitchMax = 150; // this sets the maximum angle of the pitch servo to prevent
                    // it from crashing, it should remain below 180, and be
                    // greater than the pitchMin
int pitchMin =
    33; // this sets the minimum angle of the pitch servo to prevent it from
        // crashing, it should remain above 0, and be less than the pitchMax

void shakeHeadYes(int moves = 3); // function prototypes for shakeHeadYes and No
                                  // for proper compiling
void shakeHeadNo(int moves = 3);

int yawServoVal =
    90; // initialize variables to store the current value of each servo
...
void upMove(int moves) { // function to tilt up
  for (int i = 0; i < moves; i++) {
    if ((pitchServoVal + pitchMoveSpeed) <
        pitchMax) { // make sure the servo is within rotation limits (less than
                    // 150 degrees by default)
...
void shakeHeadYes(
    int moves = 3) { // sets the default number of nods to 3, but you can pass
                     // in whatever number of nods you want
  Serial.println("YES");
